...
class Nav2PointRuleNode(RuleNode):
    ...
    def find_neighbors(
        self, waypoints: List[Point], current_wp: Point, step=2
    ) -> List[Point]:
        neighbors = []
        for wp in waypoints:
    ...
    def is_path_clear(self, start: Position2D, end: Position2D) -> bool:
        ...
        start_i = int(
            (start[1] - self.map.info.origin.position.y) / self.map.info.resolution
        )
        start_j = int(
            (start[0] - self.map.info.origin.position.x) / self.map.info.resolution
        )
        end_i = int(
            (end[1] - self.map.info.origin.position.y) / self.map.info.resolution
        )
        end_j = int(
            (end[0] - self.map.info.origin.position.x) / self.map.info.resolution
        )
    ...
    def find_path_through_waypoints(self, start: Point, target: Point) -> List[Point]:
        ...
        while queue:
            ...
            for neighbor in neighbors:
                ...
                if neighbor_pos not in visited and self.is_path_clear(
    ...
    def control_cycle(self, _):
        ...
        if self.point.x != self.prev_point.x or self.point.y != self.prev_point.y:
            ...
            for wp in self.waypoints:
                if dist is None or calc_distance(start, wp) < dist:
            ...
            for wp in self.waypoints:
                if dist is None or calc_distance(self.point, wp) < dist:
                    dist = calc_distance(self.point, wp)
                    closer_2_target = wp

            self.path = self.find_path_through_waypoints(
                closer_2_start, closer_2_target
            )
