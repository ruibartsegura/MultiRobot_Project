...
def a_star(start_m, goal_m, gmap, movement="8N", occupancy_cost_factor=3):
    ...
    visited = np.zeros(gmap.shape(), dtype="bool")
    ...
    while front:
        ...
        for dx, dy, deltacost in movements:
            ...
            if (not visited[new_pos]) and (not gmap.is_occupied_idx(new_pos)):
                cost = 1.0
                potential_function_cost = cost * occupancy_cost_factor
    ...
    if pos == goal:
        while pos:
            ...
            pos_m_x, pos_m_y = gmap.get_coordinates_from_index(pos)
